...
def parse_args():
    ...
    parser.add_argument(
        '--model', dest='model', help='Path of model for evaluation.',  required=True,
        default='', type=str)
    parser.add_argument(
        '--backbone', dest='backbone', help='Model backbone',
        default='', type=str)
    parser.add_argument(
        '--save_result', dest='save_result',
    ...
    parser.add_argument(
        '--test_dir', dest='test_dir', help='Directory path for training data.',
        default='./random_split/test', type=str)
    parser.add_argument(
    ...
    parser.add_argument(
        '--use_gpu', type=int, default=0)
    parser.add_argument(
        '--gpu', default='', type=str)
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for order, batch in enumerate(test_loader):
            ...
            count += batch_size
            if order % 10 == 0:
                print(f"Current Iter-{order}: top1_acc - {top1_acc/count:.6f} / top5_acc - {top5_acc/count:.6f}")
